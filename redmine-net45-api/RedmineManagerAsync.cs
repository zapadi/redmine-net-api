...
//
        public async Task<User> GetCurrentUserAsync(NameValueCollection parameters = null)
        {
            ...
            var result = await new RedmineWebClientAsync(apiKey).DownloadStringAsync(uri);

			return await DeserializeAsync<User>(result);
        }
//
        public async Task<WikiPage> GetWikiPageAsync(string projectId, NameValueCollection parameters, string pageName, uint version = 0)
        {
			...
			return await DeserializeAsync<WikiPage>(result);
        }
//
...
//        }
		...
		public  async Task<T> DeserializeAsync<T>( HttpResponseMessage result)
		{
			
			return await DeserializeAsync<T> (result);
		}

		public  async Task<T> DeserializeAsync<T>( HttpResponseMessage result,  string jsonRoot = null)
		{
			int totalItems = 0;
			...
			if (result.IsSuccessStatusCode) {
				{
					...
					if (type == typeof(List<T>)) {
						if (mimeFormat == MimeFormat.json) {
							...
							return (T)RedmineSerialization.JsonDeserializeToList (r, jsonRoot, type, out totalItems);
						}
						...
						if (mimeFormat == MimeFormat.json) {
							return (T)RedmineSerialization.JsonDeserialize (r, type, jsonRoot);
						}

						return (T)RedmineSerialization.FromXML (r, type);
					}
				}
			} else
				throw new RedmineException (result.ReasonPhrase);
		}
    }
}
...
namespace xUnitTestredminenet45api
{
    ...
    public class UserAsyncTests
    {
        ...
        {
            var currentUser = await fixture.Manager.GetCurrentUserAsync();
        ...
        {
            var user = await fixture.Manager.GetObjectAsync<User>(USER_ID, null);
            Assert.NotNull(user);
        }
        ...
        public async Task Should_Get_User_By_Id_Including_Groups_And_Memberships()
        {
            var user =
                await
                    fixture.Manager.GetObjectAsync<User>(USER_ID,
                        new NameValueCollection {{RedmineKeys.INCLUDE, "groups,memberships"}});
        ...
        {
            var result = await fixture.Manager.GetPaginatedObjectsAsync<User>(new NameValueCollection
            {
        ...
        public async Task Should_Get_All_Users_With_Groups_And_Memberships()
        {
            var users =
                await
                    fixture.Manager.GetObjectsAsync<User>(new NameValueCollection
                    {
                        {RedmineKeys.INCLUDE, "groups, memberships"}
                    });
        ...
        {
            var users = await fixture.Manager.GetObjectsAsync<User>(new NameValueCollection
            {
        ...
        {
            await fixture.Manager.AddUserToGroupAsync(GROUP_ID, int.Parse(USER_ID));

            var user = fixture.Manager.GetObject<User>(USER_ID.ToString(CultureInfo.InvariantCulture),
        ...
        {
            await fixture.Manager.DeleteUserFromGroupAsync(GROUP_ID, int.Parse(USER_ID));

            var user =
                await
                    fixture.Manager.GetObjectAsync<User>(USER_ID.ToString(CultureInfo.InvariantCulture),
                        new NameValueCollection {{RedmineKeys.INCLUDE, RedmineKeys.GROUPS}});
        ...
        public async Task Should_Create_User()
        {
            ...
            user.CustomFields.Add(new IssueCustomField
            {
                Id = 4,
                Values =
                    new List<CustomFieldValue> {new CustomFieldValue {Info = "userTestCustomField:" + DateTime.UtcNow}}
            });

            var createdUser = await fixture.Manager.CreateObjectAsync(user);
        ...
        public async Task Should_Update_User()
        {
            ...
            var user = fixture.Manager.GetObject<User>(userId, null);
            user.FirstName = "modified first name";
            await fixture.Manager.UpdateObjectAsync(userId, user);

            var updatedUser = await fixture.Manager.GetObjectAsync<User>(userId, null);
        ...
        public async Task Should_Delete_User()
        {
            ...
            await fixture.Manager.DeleteObjectAsync<User>(userId, null);
            await
                Assert.ThrowsAsync<NotFoundException>(
                    async () => await fixture.Manager.GetObjectAsync<User>(userId, null));
        }
    }
}
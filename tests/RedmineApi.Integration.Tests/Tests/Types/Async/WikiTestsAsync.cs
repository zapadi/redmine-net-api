...
public class WikiTestsAsync(RedmineTestContainerFixture fixture)
{
    ...
    public async Task UpdateWikiPage_Should_Succeed()
    {
        ...
        if (initialPage is not null)
        {
             retrievedPage = await fixture.RedmineManager.GetAsync<WikiPage>(initialPage.Id.ToInvariantString());
        }
        else
        {
            retrievedPage = await fixture.RedmineManager.GetWikiPageAsync(PROJECT_ID, pageTitle);
        }

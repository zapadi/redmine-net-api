...
internal static class RandomHelper
{
    ...
    /// <param name="lowerCaseOnly"></param>
    /// <returns>A random string composed of English alphabet characters.</returns>
    private static string GenerateRandomString(int length = 10, bool? lowerCaseOnly = null)
    {
        if (length <= 0)
        {
            throw new ArgumentException(nameof(length), "Length must be a positive integer.");
        }
        ...
        {
            var ch = EnglishAlphabetChars[random.Next(EnglishAlphabetChars.Length)];
            if (lowerCaseOnly is true)
            {
                if (char.IsUpper(ch))
                {
                    ch = (char)(ch | 0x20);
    ...
    public static string GenerateText(int length = 10, bool? lowerCase = null)
    {

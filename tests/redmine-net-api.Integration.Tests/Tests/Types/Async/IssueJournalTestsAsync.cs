...
public class IssueJournalTestsAsync(RedmineTestContainerFixture fixture)
{
    ...
    public async Task GetIssueWithJournals_Should_Succeed()
    {
        ...
        await fixture.RedmineManager.UpdateAsync(createdIssue.Id.ToString(), updateIssue, cancellationToken: TestContext.Current.CancellationToken);

        // Act
        // Get the issue with journals
        var retrievedIssue =
            await fixture.RedmineManager.GetAsync<Issue>(createdIssue.Id.ToString(), RequestOptions.Include("journals"), TestContext.Current.CancellationToken);

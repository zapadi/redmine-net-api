...
namespace Padi.RedmineApi.Serialization.Xml.Cache;
...
internal sealed class XmlSerializerCache : IXmlSerializerCache
{
	...
	/// any previously cached instances are compatible
	...
	public XmlSerializer? GetSerializer(Type type, XmlAttributeOverrides? overrides, Type[] types, XmlRootAttribute? root, string? defaultNamespace)
	{
		...
		lock (_syncRoot)
		{
			if (!_serializers.TryGetValue(key, out var cachedSerializer))
			{
				lock (_syncRoot)
				{

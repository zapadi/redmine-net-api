...
using Padi.RedmineApi.Serialization.JsonText.Converters;
...
internal static class JsonConverterRegistry
{
    ...
    public static IJsonTextRedmineConverter<T> GetConverter<T>()
    {
        if (Converters.TryGetValue(typeof(T), out var converter))
        {
            return (IJsonTextRedmineConverter<T>)converter;
        }
